	.include "test_defines.h"

	.text
	.align	2
	
	.globl	_start
_start:

	TEST_INIT

	li		t2, 0xf0e0d0c0b0a09080

	# Zero offset, base doubleword aligned

	la		t0, mem01+8
	sb		t2, 0(t0)
	la		t3, mem01
	ld		t1, 8(t3)
	TEST_EXPECT	t1, 0x3f3e3d3c3b3a3980

	# Positive offset, base doubleword aligned

	la		t0, mem02+8
	sb		t2, 2(t0)
	la		t3, mem02
	ld		t1, 8(t3)
	TEST_EXPECT	t1, 0x3f3e3d3c3b803938

	# Negative offset, base doubleword aligned

	la		t0, mem03+8
	sb		t2, -2(t0)
	la		t3, mem03
	ld		t1, 0(t3)
	TEST_EXPECT	t1, 0x3780353433323130

	# Zero offset, base not doubleword aligned

	la		t0, mem04+7
	sb		t2, 0(t0)
	la		t3, mem04
	ld		t1, 0(t3)
	TEST_EXPECT	t1, 0x8036353433323130

	# Positive offset, base not doubleword aligned

	la		t0, mem05+7
	sb		t2, 2(t0)
	la		t3, mem05
	ld		t1, 8(t3)
	TEST_EXPECT	t1, 0x3f3e3d3c3b3a8038

	# Negative offset, base not doubleword aligned

	la		t0, mem06+7
	sb		t2, -2(t0)
	la		t3, mem06
	ld		t1, 0(t3)
	TEST_EXPECT	t1, 0x3736803433323130


	TEST_END

	.data
	.align	2
	
mem01:	.byte	0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37
	.byte	0x38, 0x39, 0x3a, 0x3b, 0x3c, 0x3d, 0x3e, 0x3f
	
mem02:	.byte	0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37
	.byte	0x38, 0x39, 0x3a, 0x3b, 0x3c, 0x3d, 0x3e, 0x3f
	
mem03:	.byte	0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37
	.byte	0x38, 0x39, 0x3a, 0x3b, 0x3c, 0x3d, 0x3e, 0x3f
	
mem04:	.byte	0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37
	.byte	0x38, 0x39, 0x3a, 0x3b, 0x3c, 0x3d, 0x3e, 0x3f
	
mem05:	.byte	0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37
	.byte	0x38, 0x39, 0x3a, 0x3b, 0x3c, 0x3d, 0x3e, 0x3f
	
mem06:	.byte	0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37
	.byte	0x38, 0x39, 0x3a, 0x3b, 0x3c, 0x3d, 0x3e, 0x3f
	
